 ...
 * @Author: 张小菜
 * @Description: [ SpecificationFactory ] 说明： 无
 * @Function: 功能描述： 无
 * @Date: 2023/11/18 018 23:59
 * @Version: v1.0
 */
public class SpecificationFactory {
     ...
     * @return 查询条件的封装对象
     */
    public static <T> Specification equal(String fieldName, T value) {
     ...
     * @return 查询条件的封装对象
     */
    public static <T> Specification notEqual(String fieldName, T value) {
    ...
    /**
     * or 条件 待测试
     * @param andPredicateList  and 条件
     * @param orPredicateList  or 条件
     *
        and条件主要是直接add就行
        关键是or,需要新建一个List<Predicate> ,两者之间用or进行关联
        https://www.cnblogs.com/banywl/articles/14016123.html
     * @return
     */
    public static Specification andOr(List<Predicate> andPredicateList, List<Predicate> orPredicateList){
        return (root, query, cb) -> {
            // 所有的and 加入查询条件
            Predicate predicateAnd = cb.and(andPredicateList.toArray(new Predicate[andPredicateList.size()]));
            //OR查询加入查询条件
            Predicate predicateOR = cb.or(orPredicateList.toArray(new Predicate[orPredicateList.size()]));
    ...
    https://www.pianshen.com/article/41181543619/
     */
